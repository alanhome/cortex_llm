...
[**Cortex LLM** 详细论文见pdf链接](https://alanhome.github.io/cortex-llm/Cortex_LLM.pdf) 是一个轻量级、可插拔的 **"System 2" 推理增强模块**。它不需要微调（Fine-tune）庞大的基座模型（Base Model），而是通过挂载一个可学习的“大脑皮层”（Cortex），在冻结的 LLM 之上通过生成“思维向量（Thought Vectors）”，从而引导模型进行更深层次的推理。
